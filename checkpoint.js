...
suma= array.length[0]+array.length[i];
if(suma===n){
correcto= true;
}  }
...
return arreglo=array.map(lista=>lista[propiedad]);
}
  ...
  class Persona {
   ...
   this.amigos.push({nombre: nombre, edad: edad});
    ...
    getHobbies() {
      ...
      // Tu código aca:
      return this.hobbies.map (game => game);
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:
   let nuevo=0;
   const nuevoresultado= (acc, suma)=>acc+suma;
   nuevo=this.amigos.map (amigo => amigo.edad);
   nuevo= nuevo.reduce(nuevoresultado);
return nuevo/this.amigos.length
}
};
return Persona; 
...
// No logre comprender bien la consigna 
